...
//			int eoTheLastRecievedMessage = 0;
			...
			log=log+ "EoTheLastSentMessage: "+session.eoTheLastSentMessage+"\n";
			log=log+"EoTheLastRecievedMessage: "+session.eoTheLastRecievedMessage+"\n";
			log=log + "IsTransactional: "+ session.isTransactional+ "\n";
		...
		try {
			...
			if (file_ != null) {
				...
				while (log != null) {
					...
					log = reader.readLine();
					log = log.substring(log.indexOf(" ") + 1);
					session.setEoTheLastSentMessage(Integer.valueOf(log));
					log = reader.readLine();
					log = log.substring(log.indexOf(" ") + 1);
					session.setEoTheLastRecievedMessage(Integer.valueOf(log));
					log = reader.readLine();
					log = log.substring(log.indexOf(" ") + 1);
					if (log.compareTo("true")==0)
						session.setTransactional(true);
